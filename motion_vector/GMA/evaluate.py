...
import datasets
from flask.server.src.utils import flow_viz
from flask.server.src.utils import frame_utils

from flask.server.src.utils import InputPadder, forward_interpolate
    ...
    for dstype in ['clean', 'final']:
        ...
        for test_id in range(len(test_dataset)):
            ...
            image1, image2 = padder.pad(
                image1[None].to(f'cuda:{model.device_ids[0]}'),
                image2[None].to(f'cuda:{model.device_ids[0]}'))
...
def create_sintel_submission_vis(model,
    ...
    for dstype in ['clean', 'final']:
        ...
        for test_id in range(len(test_dataset)):
            ...
            imageio.imwrite(f'vis_test/gt/{dstype}/image/{test_id}.png',
...
def create_kitti_submission_vis(model, output_path='kitti_submission'):
    ...
    for test_id in range(len(test_dataset)):
        ...
        imageio.imwrite(f'vis_kitti/image/{test_id}_0.png',
                        image1[0].cpu().permute(1, 2, 0).numpy())
        imageio.imwrite(f'vis_kitti/image/{test_id}_1.png',
...
def validate_sintel_occ(model, iters=6):
    ...
    for dstype in ['albedo', 'clean', 'final']:
        # for dstype in ['clean', 'final']:
        val_dataset = datasets.MpiSintel(split='training',
                                         dstype=dstype,
                                         occlusion=True)
        epe_list = []
        epe_occ_list = []
...
def separate_inout_sintel_occ():
    ...
    for val_id in range(len(val_dataset)):
        ...
        out_of_frame = (coords_img_2[0] < 0) | (coords_img_2[0] > w) | (
            coords_img_2[1] < 0) | (coords_img_2[1] > h)
